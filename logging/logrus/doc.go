...
/*
`grpc_logrus` provides server-side interceptors and handler funcitions for using Logrus within gRPC code.
...
You can use `Extract` to log into a request-scoped `logrus.Entry` instance in your handler code. Moreover you `AddFields`
to the request-scoped `logrus.Entry`, that will be propagated for all call depending on the context, including the
...
To make sure that Logrus is also receiving the log statements from the gRPC library internals, please call
`ReplaceGrpcLogger`.
