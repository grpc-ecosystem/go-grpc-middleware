...
//
// This *only* works when placed *after* the `grpc_kit.UnaryServerInterceptor`. However, the logging can be done to a
...
//
// This *only* works when placed *after* the `grpc_kit.StreamServerInterceptor`. However, the logging can be done to a
// separate instance of the logger.
...
func logProtoMessageAsJson(logger log.Logger, pbMsg interface{}, key string, msg string) {
	if p, ok := pbMsg.(proto.Message); ok {
		...
		if err != nil {
			_ = level.Info(logger).Log(key, err)
		}
		_ = level.Info(logger).Log(key, string(payload))
	}
}
