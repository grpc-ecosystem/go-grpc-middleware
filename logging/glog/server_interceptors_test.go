...
func TestGLogServerSuite(t *testing.T) {
	if strings.HasPrefix(runtime.Version(), "go1.7") {
		t.Skipf("Skipping due to json.RawMessage incompatibility with go1.7")
		return
	}
	opts := []grpc_glog.Option{
		grpc_glog.WithLevels(customCodeToLevel),
	}
	b := newGLogBaseSuite(t)
	b.InterceptorTestSuite.ServerOpts = []grpc.ServerOption{
		grpc_middleware.WithStreamServerChain(
			grpc_ctxtags.StreamServerInterceptor(grpc_ctxtags.WithFieldExtractor(grpc_ctxtags.CodeGenRequestFieldExtractor)),
			grpc_glog.StreamServerInterceptor(ctx_glog.NewEntry(b.logger), opts...)),
		grpc_middleware.WithUnaryServerChain(
			grpc_ctxtags.UnaryServerInterceptor(grpc_ctxtags.WithFieldExtractor(grpc_ctxtags.CodeGenRequestFieldExtractor)),
			grpc_glog.UnaryServerInterceptor(ctx_glog.NewEntry(b.logger), opts...)),
	}
	suite.Run(t, &glogServerSuite{b})
}

type glogServerSuite struct {
	*glogBaseSuite
}

func (s *glogServerSuite) TestPing_WithCustomTags() {
...
func (s *glogServerSuite) TestPingError_WithCustomLevels() {
...
func (s *glogServerSuite) TestPingList_WithCustomTags() {
...
func TestGLogServerOverrideSuite(t *testing.T) {
	if strings.HasPrefix(runtime.Version(), "go1.7") {
	...
	suite.Run(t, &glogServerOverrideSuite{b})
}

type glogServerOverrideSuite struct {
	*glogBaseSuite
}

func (s *glogServerOverrideSuite) TestPing_HasOverriddenDuration() {
...
func (s *glogServerOverrideSuite) TestPingList_HasOverriddenDuration() {
	stream, err := s.Client.PingList(s.SimpleCtx(), goodPing)
	require.NoError(s.T(), err, "should not fail on establishing the stream")
	for {
		_, err := stream.Recv()
...
func TestGLogServerOverrideDeciderSuite(t *testing.T) {
	if strings.HasPrefix(runtime.Version(), "go1.7") {
		t.Skip("Skipping due to json.RawMessage incompatibility with go1.7")
		return
	}
	opts := []grpc_glog.Option{
		grpc_glog.WithDecider(func(method string, err error) bool {
	...
	suite.Run(t, &glogServerOverrideDeciderSuite{b})
}

type glogServerOverrideDeciderSuite struct {
	*glogBaseSuite
}

func (s *glogServerOverrideDeciderSuite) TestPing_HasOverriddenDecider() {
	_, err := s.Client.Ping(s.SimpleCtx(), goodPing)
	require.NoError(s.T(), err, "there must be not be an error on a successful call")
...
func (s *glogServerOverrideDeciderSuite) TestPingError_HasOverriddenDecider() {
	code := codes.NotFound
	level := ctx_glog.InfoLevel
	msg := "NotFound must remap to InfoLevel in DefaultCodeToLevel"
...
func (s *glogServerOverrideDeciderSuite) TestPingList_HasOverriddenDecider() {
	stream, err := s.Client.PingList(s.SimpleCtx(), goodPing)
	require.NoError(s.T(), err, "should not fail on establishing the stream")
	for {
		_, err := stream.Recv()
