...
import (
	...
	grpcslog "github.com/grpc-ecosystem/go-grpc-middleware/providers/slog"
)
...
func Example_initializationWithCustomLevels() {
	...
	_ = grpc.NewServer(
		grpc.ChainUnaryInterceptor(
			logging.UnaryServerInterceptor(grpcslog.InterceptorLogger(logger), opts...),
		),
		grpc.ChainStreamInterceptor(
			logging.StreamServerInterceptor(grpcslog.InterceptorLogger(logger), opts...),
		),
	)
}
...
func ExampleServerPayloadLoggingDecider() {
	...
	_ = []grpc.ServerOption{
		grpc.ChainUnaryInterceptor(
			logging.UnaryServerInterceptor(grpcslog.InterceptorLogger(logger)),
			logging.PayloadUnaryServerInterceptor(grpcslog.InterceptorLogger(logger), payloadDecider, time.RFC3339),
		),
		grpc.ChainStreamInterceptor(
			logging.StreamServerInterceptor(grpcslog.InterceptorLogger(logger)),
			logging.PayloadStreamServerInterceptor(grpcslog.InterceptorLogger(logger), payloadDecider, time.RFC3339),
		),
	}
}
